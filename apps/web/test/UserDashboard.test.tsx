...
describe('UserDashboard', () => {
  ...
  it('walks through onboarding, builds a plan, searches food, and logs a portion', async () => {
    renderDashboard();
    expect(screen.getByText('برنامهٔ شخصی تو')).toBeInTheDocument();
    const next = () => fireEvent.click(screen.getByRole('button', { name: 'بعدی' }));
    next();
    fireEvent.click(screen.getByRole('button', { name: 'بهبود سلامتی' }));
    next();
    fireEvent.click(screen.getByRole('button', { name: 'عادت به ریزه‌خواری' }));
    next();
    fireEvent.click(screen.getByRole('button', { name: 'نداشتن زمان' }));
    next();
    fireEvent.click(screen.getByRole('button', { name: /زن/ }));
    next();
    fireEvent.change(screen.getByLabelText('تاریخ تولد'), { target: { value: '۱۳۷۵/۰۱/۰۱' } });
    next();
    fireEvent.change(screen.getByLabelText('قد (سانتی‌متر)'), { target: { value: '165' } });
    next();
    fireEvent.change(screen.getByLabelText('وزن فعلی (کیلوگرم)'), { target: { value: '70' } });
    next();
    next();
    fireEvent.change(screen.getByLabelText('وزن ایده‌آل (کیلوگرم)'), { target: { value: '62' } });
    next();
    fireEvent.click(screen.getByRole('button', { name: /متوسط/ }));
    next();
    fireEvent.click(screen.getByRole('button', { name: /معمولی/ }));
    next();
    fireEvent.click(screen.getByRole('button', { name: 'شیر' }));
    next();
    fireEvent.click(screen.getByRole('button', { name: 'ندارم' }));
    next();
    next();
    next();
    next();
    ...
    next();
    next();
    fireEvent.click(screen.getByRole('button', { name: 'متوجه شدم' }));
    fireEvent.click(screen.getByRole('button', { name: 'برو بریم' }));
